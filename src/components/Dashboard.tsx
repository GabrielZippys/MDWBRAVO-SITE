...
// Usa Projeto diretamente como o tipo para os itens da lista de chamados
...
// Ajustado para refletir os campos de Projeto (usando 'nome', sem 'displayableIssueId' ou 'ID')
          ...
          <table className="tabela-chamados w-full mt-4 table-auto">
            <thead>
              <tr>
                ...
                {/* Coluna Título: usa 'nome' para ordenação e exibição */}
            ...
            <tbody>
              {chamadosOrdenados.map((chamado) => { // 'chamado' agora é do tipo Projeto
                ...
                return (
                  // Use chamado.pageId como chave, pois é o UUID único da página
                  <tr key={chamado.pageId} className="hover:bg-gray-700/50 transition-colors duration-150">
                    {/* Célula Título: usa chamado.nome e o link para a página do Notion */}
                    <td className="text-xs sm:text-sm p-2 align-top truncate hover:whitespace-normal" title={chamado.nome} style={{maxWidth: '350px'}}>
                      {notionLink ? (
                        ...
                        chamado.nome // Exibe o nome mesmo se não houver link (pageId ausente)
                      )}
                    </td>
